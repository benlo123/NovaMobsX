...
public abstract class WalkingAnimal extends WalkingEntity implements Animal {
    ...
    public boolean entityBaseTick(int tickDiff) {
        ...
        if (inLoveTicks > 0) {
            ...
            if (inLoveTicks % 10 == 0) {
                Vector3 pos = new Vector3(this.x + (double) (rand.nextFloat() * this.getWidth() * 2.0F) - (double) this.getWidth(), this.y + 0.5D + (double) (rand.nextFloat() * this.getHeight()), this.z + (double) (rand.nextFloat() * this.getWidth() * 2.0F) - (double) this.getWidth());
