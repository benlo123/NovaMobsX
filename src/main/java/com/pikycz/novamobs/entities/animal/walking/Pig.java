...
public class Pig extends WalkingAnimal {
    ...
    public boolean targetOption(EntityCreature creature, double distance) {
        if (creature instanceof Player) {
            ...
            return player.spawned && player.isAlive() && !player.closed && (player.getInventory().getItemInHand().getId() == Item.CARROT || player.getInventory().getItemInHand().getId() == Item.POTATO || player.getInventory().getItemInHand().getId() == Item.BEETROOT) && distance <= 49;
        }
